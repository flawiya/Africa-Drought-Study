...
# =============================================================================
# 1. CONFIGURATION & PATH HANDLING (FIXED)
# =============================================================================

# This finds the directory where the script is located, then goes up one level to the project root
...
# Specific Data Paths 
# Note: Check if your .shp is inside a subfolder or directly in 'data'
# Standard GADM folder structure is usually: data/folder_name/file_name.shp
...
# --- DEBUGGING: Print paths to verify they exist ---
print(f"Checking GADM Path: {GADM_PATH} -> {'Found' if GADM_PATH.exists() else 'NOT FOUND'}")
print(f"Checking GEOGLAM Path: {GEOGLAM_PATH} -> {'Found' if GEOGLAM_PATH.exists() else 'NOT FOUND'}")
print(f"Checking ERA5 Path: {ERA5_PATH} -> {'Found' if ERA5_PATH.exists() else 'NOT FOUND'}")

if not GADM_PATH.exists():
    raise FileNotFoundError(f"CRITICAL ERROR: Shapefile not found at {GADM_PATH}. Check folder names!")
