...
# Metric 2: Count of Extreme Drought Days (SSI <= -1.5)
...
gdf_simple = gdf.copy()
gdf_simple['geometry'] = gdf_simple['geometry'].simplify(tolerance=0.01)
zambia_geojson = json.loads(gdf_simple.geometry.to_json())

map_df_sorted = df_yearly_summary.sort_values(by=['year', 'feature_id'])

fig_anim = px.choropleth(
    map_df_sorted, geojson=zambia_geojson, locations='feature_id',
    color='Count_Extreme_Drought_Days', animation_frame='year',
    color_continuous_scale='YlOrRd', range_color=[0, 120],
    title='Zambia: Annual Pulse of Extreme Drought Days (2000-2024)'
)
fig_anim.update_geos(fitbounds="locations", visible=False)
fig_anim.write_html("Zambia_Drought_Frequency_Animation.html", include_plotlyjs="cdn")

print("✅ ALL TASKS COMPLETE. Files generated in directory.")