...
def plot_district_verification(df, nino_df, metric='SSI_seasonal'):
    ...
    fig.write_html(os.path.join(base_path, f"Southern_Province_{metric}_Verification.html"))
    # fig.show()
...
def save_styled_report(df, nino_df):
    ...
    with open(os.path.join(base_path, "Southern_Province_Validation_Report_Basic.html"), "w", encoding="utf-8") as f:
        f.write(html_out)
